...
class SegmentationLosses(CrossEntropyLoss):
    ...
    def __init__(self, DS=False, CTF=False, se_loss=False, se_weight=0.2, nclass=-1,
        ...
        super(SegmentationLosses, self).__init__(OHEM, ignore_index)
        self.DS = DS
        self.CTF = CTF
    ...
    def forward(self, *inputs):
        if self.CTF:
            ...
            loss = super(SegmentationLosses, self).forward(preds[0], target, do_OHEM=True)
            for i in range(1, logits_num):
                loss = loss + super(SegmentationLosses, self).forward(preds[i], target)
            return loss
        if self.DS:
            logits_num = len(tuple(inputs)) - 1  # target occluded
            ...
            loss = weights[0] * super(SegmentationLosses, self).forward(preds[0], target, do_OHEM=True)
            for i in range(1, logits_num):
                loss = loss + weights[i] * super(SegmentationLosses, self).forward(preds[i], target)
            return loss
