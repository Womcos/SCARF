...
class Options():
    def __init__(self):
        ...
        parser.add_argument('--dilated', action='store_true', default=False,
                            help='Use dilated convolution for backbone')
    ...
    def parse(self):
        ...
        if args.dataset.lower() in ['pcontext']:
            args.crop_size = 440
        if args.dataset.lower() == 'ade20k':
            args.crop_size = 460
        print(args)
        return args
...
def test(args):
    # output folder
    resume_pth = args.resume.split('/')[-1]
    outdir = args.resume.replace(resume_pth, 'outdir')
    ...
    if args.model_zoo is not None:
        ...
        model = get_segmentation_model(args.model, dataset=args.dataset,
                                       ...
                                       dilated=args.dilated, multi_grid=args.multi_grid,)
    ...
    scales = [1.0]
    flip = False
    whole_image = False if args.dataset == 'citys' else True
    #whole_image = True
    if args.MS:
        scales = [0.75, 1.0, 1.25, 1.5, 1.75, 2.0] if args.dataset == 'citys' else \
            [0.5, 0.75, 1.0, 1.25, 1.5, 1.75]  # , 2.0
        flip = True

    evaluator = MultiEvalModule(model, testset.num_class, scales=scales, flip=flip, whole_image=whole_image).cuda()
    evaluator.eval()
